...
export type FlatFieldProps<T extends FieldValues, P extends Path<T>> = ControllerRenderProps<T, P> & {
  id: string;
  "aria-invalid": boolean;
};

interface FieldControllerProps<T extends FieldValues, P extends Path<T> = Path<T>> {
  control: Control<T>;
  name: P;
 ...
 */
export function FieldController<T extends FieldValues, P extends Path<T> = Path<T>>({
  control,
  name,
  label,
  description,
