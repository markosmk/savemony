...
import { SpinnerIcon } from "../spinner-loader";
...
function ButtonLoading({
  ...
  return (
    <Comp
      ...
      data-variant={variant}
      data-size={size}
      className={cn(buttonVariants({ variant, size, className }), "relative grid place-items-center overflow-hidden")}
      disabled={isPending || props.disabled}
      {...props}
    >
      <span
        className={cn(
          "transition-all duration-300 ease-out flex gap-1.5",
          isPending ? "opacity-0 translate-y-2 pointer-events-none" : "opacity-100 scale-100 translate-y-0",
        )}
      >
        {children}
      </span>
      <span
        className={cn(
          ...
          isPending ? "opacity-100 translate-y-0 scale-100" : "opacity-0 scale-75 translate-y-3 pointer-events-none",
        )}
      >
        <SpinnerIcon size="sm" className="text-current" />
      </span>
    </Comp>
  );
}
